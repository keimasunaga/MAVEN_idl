...
;
;-
PRO mvn_sta_etspec_dir_kei, apid, frame=frame, units=units, thld_theta=thld_theta, attvec=attvec, $
                        trange=trange, verbose=verbose, mass=mass, suffix=suffix,$
                        rmv_contamination=rmv_contamination
  nan = !values.f_nan
  ...
  FOR i=0ll, N_ELEMENTS(time)-1 DO BEGIN ;- time loop
     ...
     if keyword_set(rmv_contamination) then sta_getdat_bgrm,d,d,ratio=0.5
