...
;
;-
PRO mvn_pfp_cotrans_kei, var, from=from, to=to, verbose=verbose, $
                     vx=vxn, vy=vyn, vz=vzn, $
                     theta=thetan, phi=phin, status=status, $
                     spice_load=spice_load, $
                     px=px, py=py, pz=pz, overwrite=overwrite,$
                     th_m_in = th_m_in, th_mp_in = th_mp_in, th_pm_in=ph_pm_in, th_pp_in = th_pp_in,$
                     ph_mm_out = ph_mm_out, th_p_out = th_p_out, ph_m_out=ph_m_out, ph_p_out = ph_p_out
...
stop
  ...
  sphere_to_cart, 1.d0, th_m_in, phi_m_in, vx_mm, vy_mm, vz_mm
 
  sphere_to_cart, 1.d0, th_m_in, ph_p_in, vx_mp, vy_mp, vz_mp
 
  sphere_to_cart, 1.d0, th_p_in, ph_m_in, vx_pm, vy_pm, vz_pm
 
  sphere_to_cart, 1.d0, th_p_in, ph_p_in, vx_pp, vy_pp, vz_pp
  ...
  vxn_mm = 2*( (t8 + t10)*vx_mm + (t6 -  t4)*vy_mm + (t3 + t7)*vz_mm ) + vx_mm
  vyn_mm = 2*( (t4 +  t6)*vx_mm + (t5 + t10)*vy_mm + (t9 - t2)*vz_mm ) + vy_mm
  vzn_mm = 2*( (t7 -  t3)*vx_mm + (t2 +  t9)*vy_mm + (t5 + t8)*vz_mm ) + vz_mm

  th_mm = 90. - ACOS(vzn_mm)*!RADEG
  ph_mm = ATAN(vyn_mm, vxn_mm) * !RADEG
  
  
  vxn_mp = 2*( (t8 + t10)*vx_mp + (t6 -  t4)*vy_mp + (t3 + t7)*vz_mp ) + vx_mp
  vyn_mp = 2*( (t4 +  t6)*vx_mp + (t5 + t10)*vy_mp + (t9 - t2)*vz_mp ) + vy_mp
  vzn_mp = 2*( (t7 -  t3)*vx_mp + (t2 +  t9)*vy_mp + (t5 + t8)*vz_mp ) + vz_mp

  th_mp = 90. - ACOS(vzn_mp)*!RADEG
  ph_mp = ATAN(vyn_mp, vxn_mp) * !RADEG
  
  
  vxn_pm = 2*( (t8 + t10)*vx_pm + (t6 -  t4)*vy_pm + (t3 + t7)*vz_pm ) + vx_pm
  vyn_pm = 2*( (t4 +  t6)*vx_pm + (t5 + t10)*vy_pm + (t9 - t2)*vz_pm ) + vy_pm
  vzn_pm = 2*( (t7 -  t3)*vx_pm + (t2 +  t9)*vy_pm + (t5 + t8)*vz_pm ) + vz_pm

  th_pm = 90. - ACOS(vzn_pm)*!RADEG
  ph_pm = ATAN(vyn_pm, vxn_pm) * !RADEG
  
  vxn_pp = 2*( (t8 + t10)*vx_pp + (t6 -  t4)*vy_pp + (t3 + t7)*vz_pp ) + vx_pp
  vyn_pp = 2*( (t4 +  t6)*vx_pp + (t5 + t10)*vy_pp + (t9 - t2)*vz_pp ) + vy_pp
  vzn_pp = 2*( (t7 -  t3)*vx_pp + (t2 +  t9)*vy_pp + (t5 + t8)*vz_pp ) + vz_pp

  th_pp = 90. - ACOS(vzn_pp)*!RADEG
  ph_pp = ATAN(vyn_pp, vxn_pp) * !RADEG
