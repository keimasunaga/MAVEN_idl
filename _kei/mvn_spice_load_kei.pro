...
pro mvn_spice_load_kei,trange=trange,kernels=kernels,download_only=download_only,verbose=verbose
...
;   tplot,var_label='orbnum'
   ...
   s = spice_file_source()
   s.no_server = 1
   kernels = mvn_spice_kernels(/all,/clear,/load,trange=trange,verbose=2,source=s)
   if keyword_set(download_only) then return
   ;cspice_furnsh,'/Volumes/Gravity/work/data/misc/spice/naif/MAVEN/kernels/spk/archive/maven_orb.bsp.arc4'
   
   spice_position_to_tplot,'MAVEN','Mars',frame='MSO',res=60d,scale=1000.,name=n1  ,trange=trange
   xyz_to_polar,n1
