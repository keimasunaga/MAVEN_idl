PRO sta_c6_bgrm_kei, counts, eflux, energy, time, ratio = ratio, mass_range=mass_range,overwrite = overwrite
...
;; Mar 2015
    ...
    if keyword_set(mass_range) then wp_rmv = where(mass lt mass_range[0] or mass gt mass_range[1])
    ...
    if wp[0] ne -1 then begin
       ...
       if keyword_set(mass_range) then begin
         eflux[wp_rmv] = 0.
         counts[wp_rmv] = 0.
       endif
