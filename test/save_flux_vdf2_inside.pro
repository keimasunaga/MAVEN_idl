...
pro save_flux_vdf2_inside,orbit,trange=trange,png=png,ps2pdf=ps2pdf,ps2png=ps2png,save=save,event=event,highres=highres,_extra=extra,get_apid=get_apid,$
                   plot_vdf=plot_vdf
  ...
  for it = 0,n_elements(time)-1 do begin
    ...
    if keyword_set(plot_vdf) then flux_vdf2,dat=dat_new,/local,/show,units='df',sta_apid=get_apid,mult='4,5',panel_mode=2,nbin=800,vres=2.,$
                  /remove,_extra=extra,mass_assmpt=16.,mass_range=[12,20],angle=[-45,45],/surf,/plot_vdf
    flux_vdf2,dat_new,/local,/show,units='df',sta_apid=get_apid,mult='4,5',panel_mode=1,nbin=800,vres=2.,$
