...
pro plot_sta_vdf_event_test,orbit,trange=trange,png=png,ps=ps,save=save,event=event,highres=highres,_extra=extra,get_apid=get_apid,len_ratio=len_ratio,time_upstrm=time_upstrm
  ...
  mvn_sta_l2_load,sta_apid=l2_load_apid,trange=trange
  mvn_sta_l2_tplot
  get_data,'mvn_sta_d0_E',data=d
  stop
  ...
  for it = 0,n_elements(time)-1 do begin
    ...
    plot_mvn_orbit,orbit=orbit,/xy,_extra=extra,xrange=[3,-3],yrange=[3,-3]
    plot_mvn_orbit,tspan,/xy,/oplot,/seconds,Evec=field.E,Bvec=field.B,Vvec=field.V,len_ratio=len_ratio
    plot_mvn_orbit,orbit=orbit,/xz,_extra=extra,xrange=[3,-3],yrange=[-3,3]
    plot_mvn_orbit,tspan,/xz,/oplot,/seconds,Evec=field.E,Bvec=field.B,Vvec=field.V,len_ratio=len_ratio
    plot_mvn_orbit,orbit=orbit,/yz,_extra=extra,xrange=[-3,3],yrange=[-3,3]
    plot_mvn_orbit,tspan,/yz,/oplot,/seconds,Evec=field.E,Bvec=field.B,Vvec=field.V,len_ratio=len_ratio
    plot_mvn_orbit,orbit=orbit,/cyl,_extra=extra,xrange=[3,-3],yrange=[0,3]
    plot_mvn_orbit,tspan,/cyl,/oplot,/seconds
    
    plot_mvn_orbit,orbit=orbit,/xy,_extra=extra,xrange=[3,-3],yrange=[3,-3],frame='MSE',angle=rot
    plot_mvn_orbit,tspan,/xy,/oplot,/seconds,Evec=field.E,Bvec=field.B,Vvec=field.V,frame='MSE',angle=rot,len_ratio=len_ratio
    plot_mvn_orbit,orbit=orbit,/xz,_extra=extra,xrange=[3,-3],yrange=[-3,3],frame='MSE',angle=rot
    plot_mvn_orbit,tspan,/xz,/oplot,/seconds,Evec=field.E,Bvec=field.B,Vvec=field.V,frame='MSE',angle=rot,len_ratio=len_ratio
    plot_mvn_orbit,orbit=orbit,/yz,_extra=extra,xrange=[-3,3],yrange=[-3,3],frame='MSE',angle=rot
    plot_mvn_orbit,tspan,/yz,/oplot,/seconds,Evec=field.E,Bvec=field.B,Vvec=field.V,frame='MSE',angle=rot,len_ratio=len_ratio
    plot_mvn_orbit,orbit=orbit,/cyl,_extra=extra,xrange=[3,-3],yrange=[0,3]
...
;    plot_sta_vdf,trange=tspan,/local,/show,units='df',sta_apid='d0',mult='4,3',panel_mode=1,/remove,vdf_sav=vdf_sav
    plot_sta_vdf,dat=dat_new,/local,/show,units='df',sta_apid=get_apid,mult='4,5',panel_mode=1,nbin=800,vres=2.,$
                 /remove,vdf_sav=vdf_sav,_extra=extra,mass_assmpt=16.,mass_range=[12,20],angle=[-45,45],/surf,time_upstrm=time_upstrm
    ...
    if keyword_set(ps) then begin
      pclose
      if not keyword_set(highres) then begin
        spawn,'pstopdf '+ps_path+'/vdf_'+date_s+'.eps';+SAVE_LOC+'/maven/ps/vdf/normal/event/'+string(orbit,format='(i05)')+'/.eps'
        spawn,'rm ' + ps_path+'/vdf_'+date_s+'.eps';SAVE_LOC+'/maven/ps/vdf/normal/event/'+string(orbit,format='(i05)')+'.eps'
      endif else begin
        spawn,'pstopdf '+ps_path+'/vdf_'+date_s+'.eps';SAVE_LOC+'/maven/ps/vdf/highres/event/'+string(orbit,format='(i05)')+'.eps'
        spawn,'rm ' + ps_path+'/vdf_'+date_s+'.eps';SAVE_LOC+'/maven/ps/vdf/higres/event/'+string(orbit,format='(i05)')+'.eps'
      endelse
    endif
    
    if keyword_set(save) then begin
     if not keyword_set(highres) then begin
       if keyword_set(event) then save_path = SAVE_LOC_HEAVY+'/maven/sav/vdf/normal/event/'+string(orbit,format='(i05)') $
                             else save_path = SAVE_LOC_HEAVY+'/maven/sav/vdf/normal/oxy/'+string(orbit,format='(i05)')
     endif else begin
       if keyword_set(event) then save_path = SAVE_LOC_HEAVY+'/maven/sav/vdf/highres/event/'+string(orbit,format='(i05)') $
                             else save_path = SAVE_LOC_HEAVY+'/maven/sav/vdf/highres/oxy/'+string(orbit,format='(i05)')
