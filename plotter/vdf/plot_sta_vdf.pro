pro save_mse_info,time,pos,E_c_mse,vx_mse,vy_mse,vz_mse
   ...
   ndat = n_elements(Vx_mse)
   Xmse = replicate(pos[0],ndat)
   Ymse = replicate(pos[1],ndat)
   Zmse = replicate(pos[2],ndat)
   
   sav = [[Xmse],[Ymse],[Zmse],[E_c_mse],[vx_mse],[vy_mse],[vz_mse]] 
   path = SAVE_LOC + '/maven/ascii/vdfparam/event/'+string(round(orbit),format='(i05)')
   if file_test(path) eq 0 then file_mkdir,path
   filename = path + '/'+ 'param_'+date_s+'.dat'
   format = '7(f17.10)'
   write_ascii,sav,filename,format

end

pro vbvec,trange,vb,solar_wind=solar_wind,local=local,mva=mva,nearest=nearest
...
pro plot_sta_vdf,trange=trange,load=load,sta_apid=sta_apid,dat=dat,$
                 ...
                 solar_wind=solar_wind,local=local,mva=mva,mso=mso,mse=mse,nearest=nearest,$
                 mult=mult,panel_mode=panel_mode,vdf_sav=vdf_sav,charsize=charsize,$
                 nbin=nbin,vres=vres,vrange=vrange,save_mse_info=save_mse_info,$
                 time_upstrm=time_upstrm
    ...
    vbvec,[dat.time,dat.end_time],vb,solar_wind=solar_wind,local=local,mva=mva,nearest=nearest
    ...
    ;time_upstrm=time_double(['2014-12-23/04:56:27','2014-12-23/04:58:35'])
    if keyword_set(time_upstrm) then begin
    ;; get velocity vector and magnetic field vector of solar wind or local flow.
    ;; a keyword of mva denotes the minimum variance analysis, which is useful to determine magentic field direction in the plasma sheet.
    vbvec,time_upstrm,vb_upstrm,/local
    ;dens = vb_upstrm.dens & dens_std = vb_upstrm.dens_std
    Vupstrm = vb_upstrm.Vmso & Vupstrm_e = Vupstrm/total(Vupstrm^2)^.5 & Vupstrm_std = vb_upstrm.Vmso_std
    ;th_v = vb_upstrm.theta_v & th_v_std = vb_upstrm.theta_v_std & ph_v = vb_upstrm.phi_v & r_v = vb_upstrm.r_v & z_v = vb_upstrm.ztest_v
    Bupstrm = vb_upstrm.Bmso & Bupstrm_e = Bupstrm/total(Bupstrm^2)^.5 & Bupstrm_std = vb_upstrm.Bmso_std
    ;th_b = vb_upstrm.theta_b & th_b_std = vb_upstrm.theta_b_std & ph_b = vb_upstrm.phi_b & r_b = vb_upstrm.r_b & z_b = vb_upstrm.ztest_b

    vupstrm_perp_tot = vb_upstrm.vperp & vupstrm_para_tot = vb_upstrm.vpara
    endif
    ...
    mvn_sta_convert_units_kei_v2,dat,units ;'df'
       ...
       for ip=0,3 do begin
         ...
         for ia=0,15 do begin
            ...
            ;; convert 8 points into the bve coordinate system 
             if not keyword_set(mso) and not keyword_set(mse) then begin
            ...
            if keyword_set(mse) then begin
              rot_agl = get_rot_angle(Vmso,Bmso)
              v_mmm = mso2mse([u_mmm_x],[u_mmm_y],[u_mmm_z], rot_agl)
              v_mmp = mso2mse([u_mmp_x],[u_mmp_y],[u_mmp_z], rot_agl)
              v_mpm = mso2mse([u_mpm_x],[u_mpm_y],[u_mpm_z], rot_agl)
              v_mpp = mso2mse([u_mpp_x],[u_mpp_y],[u_mpp_z], rot_agl)
              v_pmm = mso2mse([u_pmm_x],[u_pmm_y],[u_pmm_z], rot_agl)
              v_pmp = mso2mse([u_pmp_x],[u_pmp_y],[u_pmp_z], rot_agl)
              v_ppm = mso2mse([u_ppm_x],[u_ppm_y],[u_ppm_z], rot_agl)
              v_ppp = mso2mse([u_ppp_x],[u_ppp_y],[u_ppp_z], rot_agl)  
              v_c = mso2mse(vec_c[0],vec_c[1],vec_c[2], rot_agl)
              
              Vt_mse = sqrt(V_c[0]^2 + V_c[1]^2 + V_c[2]^2)
              Vx_mse = [Vx_mse, V_c[0]/Vt_mse]
              Vy_mse = [Vy_mse, V_c[1]/Vt_mse]
              Vz_mse = [Vz_mse, V_c[2]/Vt_mse]
              E_c_mse = [E_c_mse,E_c[ie,ip,ia,6]]
              vx = [v_mmm[0],v_mmp[0],v_mpm[0],v_mpp[0],v_pmm[0],v_pmp[0],v_ppm[0],v_ppp[0]]
              vy = [v_mmm[1],v_mmp[1],v_mpm[1],v_mpp[1],v_pmm[1],v_pmp[1],v_ppm[1],v_ppp[1]]
              vz = [v_mmm[2],v_mmp[2],v_mpm[2],v_mpp[2],v_pmm[2],v_pmp[2],v_ppm[2],v_ppp[2]]
              vpara = vx
    ...
    rot_agl = get_rot_angle(Vmso,Bmso)
    pos_mse = mso2mse(pos[0],pos[1],pos[2],rot_agl)
    ...
    if keyword_set(time_upstrm) then begin
    vupstrmdir_bve = mso2bve(vupstrm,Vmso,Bmso)
    Vupstrmdir_bv = [vupstrmdir_bve[0],vupstrmdir_bve[1]]/(vupstrmdir_bve[0]^2+vupstrmdir_bve[1]^2)^.5
    Vupstrmdir_ve = [vupstrmdir_bve[1],vupstrmdir_bve[2]]/(vupstrmdir_bve[1]^2+vupstrmdir_bve[2]^2)^.5
    Vupstrmdir_be = [vupstrmdir_bve[0],vupstrmdir_bve[2]]/(vupstrmdir_bve[0]^2+vupstrmdir_bve[2]^2)^.5
    endif
    ...
    if keyword_set(time_upstrm) then begin
      arrow,vupstrmdir_ve[0]*650,vupstrmdir_ve[1]*650,vupstrmdir_ve[0]*750,vupstrmdir_ve[1]*750,color=80,/data,hsize=3
      plots,circle_v2(Vupstrmdir_bve[1],Vupstrmdir_bve[2],vupstrm_perp_tot),color=80,line=2
    endif
    ...
    vmso_tit = 'Vmso=('+string(Vmso[0],format='(f8.2)')+','+string(Vmso[1],format='(f8.2)')+','+string(Vmso[2],format='(f8.2)')+')'
    Bmso_tit = 'Bmso=('+string(Bmso[0],format='(f6.2)')+','+string(Bmso[1],format='(f6.2)')+','+string(Bmso[2],format='(f6.2)')+')'
    ...
    plotxyz,(findgen(nbin+1)-nbin/2.)*vres,findgen(nbin/2.+1)*vres,im_paraperp2,xrange=vrange,yrange=[0,vrange[1]],zrange=zrange,/zlog,/add,mpanel=mpanel_imparaperp,xtit='Vpara [km/s]',ytit='Vperp [km/s]',tit=vmso_tit,charsize=charsize
    ...
    loadct2,39
    plotxyz,(findgen(nbin+1)-nbin/2.)*vres,findgen(nbin/2.+1)*vres,N_im_paraperp2,xrange=vrange,yrange=[0,vrange[1]],zrange=[0,100],/add,mpanel=mpanel_Nparaperp,xtit='Vpara [km/s]',ytit='Vperp [km/s]',tit=bmso_tit,charsize=charsize
    plots,vproton_para_tot,vproton_perp_tot,psym=4,thick=2,color=230
    plots,vproton_para_tot/sqrt(mass_assmpt),vproton_perp_tot/sqrt(mass_assmpt),psym=4,thick=2,color=200
    oplot,[0,0],[0,2*vproton_perp_tot],line=2,color=230    
    loadct2,0
    ...
    oplot,mvn_ev2km(1000.,mass_assmpt)*cos(findgen(180)*!DTOR),mvn_ev2km(1000.,mass_assmpt)*sin(findgen(180)*!DTOR),color=180
    oplot,mvn_ev2km(10000.,mass_assmpt)*cos(findgen(180)*!DTOR),mvn_ev2km(10000.,mass_assmpt)*sin(findgen(180)*!DTOR),color=180
    xyouts,-vrange+100,vrange-100,'Vperp:'+string(vproton_perp_tot,format='(f6.2)')+' km/s',/data
    xyouts,-vrange+100,vrange-200,'B: '+string(total(Bmso^2)^.5,format='(f6.2)')+' nT',/data
    ...
    if keyword_set(save_mse_info) then save_mse_info,(dat.time+dat.end_time)/2d,pos_mse,E_c_mse,Vx_mse,Vy_mse,Vz_mse
