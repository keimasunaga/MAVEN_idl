...
pro plot_sep_vdf,trange=trange,load=load,sta_apid=sta_apid,dat=dat,$
    ...
    ;im_th_ph_mso = fltarr(360,180)
        ...
        mvn_sep_load,trange=trange,/L2;mvn_sta_get(sta_apid,tt=trange)
        dat_fov = mvn_sep_get_fov(trange,/cart)
        fov_x = dat_fov.fov[0,0,0,*]
        fov_y = dat_fov.fov[0,0,1,*]
        fov_z = dat_fov.fov[0,0,2,*]
        get_data,'MVN_SEP1F_ION_FLUX',data=flux
        get_data,'MVN_SEP1F_ION_ENERGY',data=energy
        get_data,'MVN_SEP1F_ION_DENERGY',data=denergy   
        calc,'"MVN_SEP1F_ION_EFLUX"="MVN_SEP1F_ION_FLUX"*"MVN_SEP1F_ION_ENERGY"'
        get_data,'MVN_SEP1F_ION_EFLUX',data=eflux
        
        E_c = reform(energy.y[0,*])
        dE = reform(denergy.y[0,*])
        E_p = E_c + dE/2.
        E_m = E_c - dE/2.
        mass =16.*1.67e-27
        v = sqrt(2.*E_c*1e3*1.602e-19/mass)*1e-3 ;[km/s]
        v_p = sqrt(2.*E_p*1e3*1.602e-19/mass)*1e-3 ;[km/s]
        v_m = sqrt(2.*E_m*1e3*1.602e-19/mass)*1e-3 ;[km/s]
        ...
        theta_xy = reform(acos(fov_x/sqrt(fov_x^2+fov_y^2))) 
        idx_m_xy = where(fov_y lt 0)
        if idx_m_xy[0] ne -1 then theta_xy[idx_m_xy] = -theta_xy[idx_m_xy]
        nr = 26 ; number of radii
        nt = 180 ; number of Thetas
        ; Create a vector of radii:
        r = FINDGEN(nr)/(nr-1)*7000.
        ; Create a vector of Thetas:
        theta = 2*!PI * FINDGEN(nt)/(nt-1)
        ; Create some data values to be contoured:
        z = fltarr(nt,nr);theta # eflux.y[0,*],E_c);COS(theta*3) # (r-0.5)^2
        zavg = fltarr(nt,nr)
        nstack = fltarr(nt,nr)
        idx_t = nn('MVN_SEP1F_ION_EFLUX',trange[0])
        
        ir = vxy
        for ie =0,n_elements(E_c)-1 do begin
          for in=0,n_elements(fov_x)-1 do begin
             itheta  = round(theta_xy[in]*!radeg*nt/360.)
             ; print,theta_xy[in]*!radeg,itheta
              iv = where(vxy[in,ie] gt r[0:nr-2] and vxy[in,ie] le r[1:nr-1]) 
           ;   print,vxy[in,ie],iv           
              z[itheta,iv] = z[itheta,iv] + eflux.y[idx_t,ie]
              nstack[itheta,iv] = nstack[itheta,iv] + 1.
            ;  print,nstack[itheta,iv]
;              POLAR_CONTOUR, nstack, theta, r, /FILL,c_color=findgen(256),/closed,/iso,levels=[10^((5/256.)*findgen(256))],xrange=[0,500],yrange=[0,500]
;stop
          endfor
        endfor
        !p.multi=[0,2,1]
        idx = where(nstack gt 0)
        zavg[idx] = z[idx]/nstack[idx]
        zavg = zavg < 1e5
        ; Create the polar contour plot:
        POLAR_CONTOUR, zavg, theta, r, /FILL,c_color=findgen(256),/iso,levels=[10^((5/256.)*findgen(256))],xrange=[-1000,1000],yrange=[-1000,1000]
        
        POLAR_CONTOUR, nstack, theta, r, /FILL,c_color=findgen(256),/iso,levels=[10^((3/256.)*findgen(256))],xrange=[-1000,1000],yrange=[-1000,1000]
...
;    endif
    ...
    vbvec,trange,vb,solar_wind=solar_wind,/local,mva=mva
    ...
    stop
    for ie=0,n_elements(E_c)-1 do begin
      mass = mass_assumpt*1.67e-27
      v = sqrt(2.*E_c[ie]*1e3*1.602e-19/mass)*1e-3 ;[km/s]
      for in=0,n_elements(fov_x)-1 do begin
        vec_bve = mso2bve([[fov_x[in]],[fov_y[in]],[fov_z[in]]],Vmso,Bmso)
        vx_arr = [vx_arr,v*vec_bve[0]]
        vy_arr = [vy_arr,v*vec_bve[1]]
        vz_arr = [vz_arr,v*vec_bve[2]]
