...
pro plot_sta_vdf_getvel,trange=trange,load=load,sta_apid=sta_apid,dat=dat,$
                 mass_range=mass_range,mass_assmpt=mass_assmpt,erange=erange,angle_range=angle_range,units=units,$
                 emplot=emplot,show_ring=show_ring,backscat=backscat,remove_proton=remove_proton,$
    ...
    idx_pos = nn('Xmso',time)
    x_mso = Xmso.y[idx_pos]
    y_mso = Ymso.y[idx_pos]
    z_mso = Zmso.y[idx_pos]
    rot_2mse = get_rot_angle(Vmso,Bmso)
    r_mse = mso2mse(x_mso,y_mso,z_mso,rot_2mse)
    x_mse = r_mse[0]
    y_mse = r_mse[1]
    z_mse = r_mse[2]
    ...
    Vmso_y = Vmso[1]
    Vmso_z = Vmso[2]
    Vmse = mso2mse(Vmso_x,Vmso_y,Vmso_z,rot_2mse)
    Vmse_x = Vmse[0]
    Vmse_y = Vmse[1]
    Vmse_z = Vmse[2]
    ...
    Bmso_y = Bmso[1]
    Bmso_z = Bmso[2]
    Bmse = mso2mse(Bmso_x,Bmso_y,Bmso_z,rot_2mse)
    Bmse_x = Bmse[0]
    Bmse_y = Bmse[1]
    Bmse_z = Bmse[2]    
    
    Emso = -crossp(Vmso*1e3,Bmso*1e-9)
    Emso_x = Emso[0]
    Emso_y = Emso[1]
    Emso_z = Emso[2]    
    Emse = mso2mse(Emso_x,Emso_y,Emso_z,rot_2mse)
    Emse_x = Emse[0]
    Emse_y = Emse[1]
    Emse_z = Emse[2]    
         ...
         for ia=0,15 do begin
            ...
            ;; convert 8 points into the bve coordinate system 
             if not keyword_set(mso) then begin
              ...
              v_c = mso2bve([vec_c*vel_c[ie,ip,ia,6]], Vmso, Bmso)
   ...
   if v_c[2]^2 gt 1.6^2*vproton_perp_tot^2 - (v_c[1] - vproton_perp_tot)^2 then begin
        ...
        if total(counts2[ie,ip,ia,*]) gt 0.1 then begin
          
                  
          ux_mso = u_c.ux[ie,ip,ia,6]
          uy_mso = u_c.uy[ie,ip,ia,6]
          uz_mso = u_c.uz[ie,ip,ia,6]
          utot = sqrt(ux_mso^2+uy_mso^2+uz_mso^2)
          ux_mso_arr = [ux_mso_arr,ux_mso/utot]  ;;mso
          uy_mso_arr = [uy_mso_arr,uy_mso/utot]  ;;mso
          uz_mso_arr = [uz_mso_arr,uz_mso/utot]  ;;mso
          r_mse = mso2mse(x_mso, y_mso,z_mso,rot_2mse)
          u_mse = mso2mse(ux_mso, uy_mso,uz_mso,rot_2mse)
          ux_mse_arr = [ux_mse_arr,u_mse[0]/utot]  ;;mse
          uy_mse_arr = [uy_mse_arr,u_mse[1]/utot]  ;;mse
          uz_mse_arr = [uz_mse_arr,u_mse[2]/utot]  ;;mse
          
          E_c_arr = [E_c_arr, E_c[ie,ip,ia,6]]
          Flux_arr = [Flux_arr, total(counts2[ie,ip,ia,*])]                
    ...
    ux_mso_arr = ux_mso_arr[1:*]
    uy_mso_arr = uy_mso_arr[1:*]
    uz_mso_arr = uz_mso_arr[1:*]
    
    ux_mse_arr = ux_mse_arr[1:*]
    uy_mse_arr = uy_mse_arr[1:*]
    uz_mse_arr = uz_mse_arr[1:*]
    
    E_c_arr = E_c_arr[1:*]
    Flux_arr = Flux_arr[1:*]
    
    Rm = 3389.9D
    x_mso_arr = replicate(x_mso,n_elements(ux_mso_arr)) * Rm
    y_mso_arr = replicate(y_mso,n_elements(ux_mso_arr)) * Rm
    z_mso_arr = replicate(z_mso,n_elements(ux_mso_arr)) * Rm
    x_mse_arr = replicate(x_mse,n_elements(ux_mso_arr)) * Rm
    y_mse_arr = replicate(y_mse,n_elements(ux_mso_arr)) * Rm
    z_mse_arr = replicate(z_mse,n_elements(ux_mso_arr)) * Rm   
    
    Vmso_x_arr = replicate(Vmso_x,n_elements(ux_mso_arr))
    Vmso_y_arr = replicate(Vmso_y,n_elements(ux_mso_arr))
    Vmso_z_arr = replicate(Vmso_z,n_elements(ux_mso_arr))
    Vmse_x_arr = replicate(Vmse_x,n_elements(ux_mso_arr))
    Vmse_y_arr = replicate(Vmse_y,n_elements(ux_mso_arr))
    Vmse_z_arr = replicate(Vmse_z,n_elements(ux_mso_arr))    
    
    Bmso_x_arr = replicate(Bmso_x,n_elements(ux_mso_arr))
    Bmso_y_arr = replicate(Bmso_y,n_elements(ux_mso_arr))
    Bmso_z_arr = replicate(Bmso_z,n_elements(ux_mso_arr))
    Bmse_x_arr = replicate(Bmse_x,n_elements(ux_mso_arr))
    Bmse_y_arr = replicate(Bmse_y,n_elements(ux_mso_arr))
    Bmse_z_arr = replicate(Bmse_z,n_elements(ux_mso_arr))
    
    Emso_x_arr = replicate(Emso_x,n_elements(ux_mso_arr))
    Emso_y_arr = replicate(Emso_y,n_elements(ux_mso_arr))
    Emso_z_arr = replicate(Emso_z,n_elements(ux_mso_arr))
    Emse_x_arr = replicate(Emse_x,n_elements(ux_mso_arr))
    Emse_y_arr = replicate(Emse_y,n_elements(ux_mso_arr))
    Emse_z_arr = replicate(Emse_z,n_elements(ux_mso_arr))
    ...
    if units eq 'flux' then zrange = [1e-1,1e4];[1e-4,1e2]
    ...
    vdf_sav = [ [x_mso_arr],[y_mso_arr],[z_mso_arr],[ux_mso_arr],[uy_mso_arr],[uz_mso_arr],[E_c_arr],[Flux_arr],[Vmso_x_arr],[Vmso_y_arr],[Vmso_z_arr],[Bmso_x_arr],[Bmso_y_arr],[Bmso_z_arr],[Emso_x_arr],[Emso_y_arr],[Emso_z_arr],$
                [x_mse_arr],[y_mse_arr],[z_mse_arr],[ux_mse_arr],[uy_mse_arr],[uz_mse_arr],[E_c_arr],[Flux_arr],[Vmse_x_arr],[Vmse_y_arr],[Vmse_z_arr],[Bmse_x_arr],[Bmse_y_arr],[Bmse_z_arr],[Emse_x_arr],[Emse_y_arr],[Emse_z_arr]]
