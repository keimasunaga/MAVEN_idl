...
pro ptcal_04_mvn,r0,ene0,azv0,elev0,oplot=oplot
...
if ~keyword_set(r0) then r0 = [0.d0,0.d0,Alt_bs]           ;- initial location

if ~keyword_set(ene0) then ene0 = 20000.d0                  ;- initial energy [eV]
if ~keyword_set(azv0) then azv0 = 180.d0/!radeg             ;- initial azimuthal angle
if ~keyword_set(elev0) then elev0 = 90.d0/!radeg             ;- initial elevation angle
...
for i=0,Nmax-1 do begin
      ...
      if r[2] lt Alt_bs then begin
        Bvec = B0
        Evec = E0
      endif
      if r[2] ge Alt_bs and r[2] lt Alt_imb then begin
        Vms = [-(-3/35.*(r[2]-Alt_imb)+100.d3), 0.d3, 0.d3]
        Bvec = Bms
        Evec = -crossp(Vms,Bms)
      endif
      if r[2] ge Alt_imb then begin
...
;----------------------------------------
;----------------------------------------
idx_bs = where(r_rec[*,2] lt 6500e3)
!p.multi=[0,1,2]
plot,r_rec[*,0]/1.d3,r_rec[*,2]/1.d3,/iso,xrange=[20000,-20000],yrange=[0,20000],xtit='[km]',ytit='[km]'
oplot,!x.crange,[Alt_bs/1.d3,Alt_bs/1.d3],color=230
oplot,!x.crange,[Alt_imb/1.d3,Alt_imb/1.d3],color=80
stop


plot,v_rec[*,0]/1.d3,v_rec[*,2]/1.d3,/iso,xrange=[1000,-1000],yrange=[-1000,1000]
...
plots,[0],[0],psym=1
oplot,[v_rec[idx_bs[0],0]]/1e3,[v_rec[idx_bs[0],2]]/1e3,color=230,psym=5
stop
