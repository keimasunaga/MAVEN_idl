...
pro ptcal_trag_01,r0,ene0,azv0,elev0,oplot=oplot,only_trag=only_trag,only_vdf=only_vdf,color=color
...
m = 16.d*mi                          ;- mass
q = e                           ;- charge
Vsw0 = [-336.2d3,49.64d3, -24.8d3]       ;- [m/s] solar wind velocity
Vms0 = [-100.d3, 0.d3, 0.d3]
B0 = [ 3.8d-9, 2.9d-9, 0.d-9 ]   ;- [T]
Bms = [ 0.d-9, 20.d-9, 0.d-9 ]
Bim = [ 0.d-9, 40.d-9, 0.d-9 ]
E0 = -crossp(Vsw0,B0)
...
Nmax = 2000                    ;- max step number

if ~keyword_set(r0) then r0 = [0.d0,0.d0,1500.d3]           ;- initial location

if ~keyword_set(ene0) then ene0 = 0.d0                  ;- initial energy [eV]
if ~keyword_set(azv0) then azv0 = 180.d0/!radeg             ;- initial azimuthal angle
if ~keyword_set(elev0) then elev0 = -60.d0/!radeg             ;- initial elevation angle
...
for i=0,Nmax-1 do begin
      ...
      if r[2] ge Alt_bs then begin
        Bvec = B0
        Evec = E0
      endif
      if r[2] lt Alt_bs and r[2] gt Alt_imb then begin
        ;Vms = [-(2/35.*(r[2]-Alt_imb)+(1./40)*r[0]+100.d3), 0.d3, 0.d3]
        Vms = [-((1./40)*r[0]+100.d3), 0.d3, 0.d3]
        Bvec = Bms
        Evec = -crossp(Vms,Bms)
      endif
      if r[2] le Alt_imb then begin
        Vms = [-100.d3, 0.d3, 0.d3]
        Bvec = Bim
        Evec = -crossp(Vms,Bms)
      endif
...
;      print,'v',v
 ...
 ;    idx2000 = where(r_rec[*,1] ge 2000e3)
  ;   idx3000 = where(r_rec[*,1] ge 2000e3)
    ...
    ;   then goto,endloop;Bvec = Bms $
     
endfor
endloop:
idx = where(r_rec[*,0] ne 0d and r_rec[*,2] ne 0d)
...
;----- plot trajectory ------------------
if keyword_set(only_traj) and keyword_set(only_vdf) then begin
...
;----------------------------------------
;----------------------------------------
idx_imb = where(r_rec[*,2] lt 1500e3)
!p.multi=[0,1,2]
if keyword_set(only_trag) then begin
if not keyword_set(oplot) then plot,r_rec[*,0]/1.d3,r_rec[*,2]/1.d3,/iso,xrange=[20000,-20000],yrange=[0,20000],xtit='[km]',ytit='[km]' $
                          else oplot,r_rec[*,0]/1.d3,r_rec[*,2]/1.d3,color=color
oplot,!x.crange,[Alt_bs/1.d3,Alt_bs/1.d3],color=230
oplot,!x.crange,[Alt_imb/1.d3,Alt_imb/1.d3],color=80
endif
stop

if keyword_set(only_vdf) then begin
if not keyword_set(oplot) then plot,v_rec[*,0]/1.d3,v_rec[*,2]/1.d3,/iso,xrange=[1000,-1000],yrange=[-1000,1000] $
                          else oplot,v_rec[*,0]/1.d3,v_rec[*,2]/1.d3,color=color
plots,[0],[0],psym=1
oplot,[v_rec[idx_imb[0],0]]/1e3,[v_rec[idx_imb[0],2]]/1e3,color=230,psym=5
oplot,[v_rec[0,0]]/1e3,[v_rec[0,2]]/1e3,color=1,psym=5
endif
...
;
;if ~keyword_set(oplot) then plot,r_rec[idx,0]/Rm,r_rec[idx,2]/Rm,/iso,xrange=[4,-5],yrange=[-5,5],/xstyle,/ystyle,charsize=2 $
;else oplot,r_rec[idx,0]/Rm,r_rec[idx,2]/Rm
