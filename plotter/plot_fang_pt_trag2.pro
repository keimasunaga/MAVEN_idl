pro plot_fang_pt_trag2
    ...
    plot,bs_pos[0,*],bs_pos[2,*],psym=1,/iso,xrange=[-3.0,3],yrange=[-5,5]
    ;plot,bs_pos[1,*],bs_pos[2,*],psym=1,/iso;,xrange=[-1,1],yrange=[-1,1]
    
    
    file_trag = SAVE_LOC + '/maven/ascii/model/fang//incident_ions/45deg/traj_I*.dat'
    ...
    for q=0,n_elements(fs)-1 do begin
      ...
      close,1
      openr,1,fs[q]
      for s=0,14 do readf,1,sline
      readf,1,dat
      close,1
      if dat[2,0] gt 0 and dat[1,0] gt 0 then begin
      oplot,dat[0,*],dat[2,*],color=230
      endif
    endfor
    stop
     plot,bs_pos[0,*],bs_pos[1,*],psym=1,/iso,xrange=[-3.0,3],yrange=[-5,5]
    ...
    plot,bs_pos[1,*],bs_pos[2,*],psym=1,/iso,xrange=[-3.0,3],yrange=[-5,5]
