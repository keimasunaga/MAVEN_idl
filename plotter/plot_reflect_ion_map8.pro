...
pro plot_reflect_ion_map8,energy,vdf_type=vdf_type,surf_type=surf_type,frange=frange,fregion=fregion,$
                          Rg_mean=Rg_mean,larger=larger,smaller=smaller,make_notes=make_notes,journal_file=journal_file
   
   

    orbit_arr = [336+findgen(500),1470+findgen(500)];346;,347,348,349,350] ;+ findgen(472)
    zrange = [1e0,1e5] 
    zrange_n = [1e0,1e3]
   ...
   rg_arr = 0.
    ...
    for in=0,n_elements(orbit_arr)-1 do begin  
      ...
      for ii=0,n_elements(fs)-1 do begin
          ...
          Bt = norm(Bmso)
          ;if Bt le 5 then goto,skip
          Emso = -crossp(Vmso,Bmso)*1e-6
          Et = norm(Emso)         
          ;if Et lt 0.9e-3 then goto,skip
          ...
          if keyword_set(Rg_mean) then begin
            if keyword_set(larger) then if Gyro_radii lt 15140.626 then goto,skip
            if keyword_set(smaller) then if Gyro_radii ge 15140.626 then goto,skip
          endif
