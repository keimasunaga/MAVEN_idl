...
if keyword_set(mass) then begin
	...
	if not keyword_set(yrange) then yrange=[.5,max(ydat)+1]  
endif else begin
	ydat=replicate(1.,mdat.nenergy)#findgen(mdat.nmass)
        if not keyword_set(ytitle) then ytitle = 'Mass Bin'
	ylog=0
	if not keyword_set(yrange) then yrange=[0,max(ydat)+1]
endelse
	ystyle=1
	yticks = 0
	ytickv = 0
...
;if keyword_set(mtable) then begin  ;  if not keyword_set(data) then begin  if keyword_set(add_mass_contour) then begin   if keyword_set(rotate) then begin     mtable = transpose(mdat.mass_arr)     m_levels = [1,2,4,12,16,32,44]     contour,mtable,ydat,xdat,levels=m_levels,/over   endif else begin     mtable = mdat.mass_arr     m_levels = [1,2,4,12,16,32,44]     contour,mtable,xdat,ydat,levels=m_levels,/over
   endelse  endif;  endif else begin;    apid = data.apid;    if apid eq 'd1' or apid eq 'd0' then begin;      if keyword_set(rotate) then begin;        mtable = mtable[*,0,*];        mtable = transpose(mtable);        m_levels = [1,2,4,12,16,32,44];        contour,mtable,ydat,xdat,levels=m_levels,/over;      endif else begin;        mtable = mtable[*,0,*];        m_levels = [1,2,4,12,16,32,44] ;        contour,mtable,xdat,ydat,levels=m_levels,/over
;      endelse;    endif
;  endelse;endif
!y.omargin = [0,0]
