pro vbvec,trange,vb,solar_wind=solar_wind,local=local,mva=mva
  ...
  if keyword_set(solar_wind) then begin
    orbit = mvn_orbit_num(time=mtime)
    filename = SAVE_LOC + '/maven/sav/solar_wind_info/15min_v1_160513/sw_info_'+string(round(orbit),format='(i05)')+'.sav' ;00426.sav'
...
pro flux_vdf7,dat,orbit=orbit,$
    ...
    file_sw = SAVE_LOC + '/maven/sav/solar_wind_info/15min_v1_160513/sw_info_'+string(orbit,format='(i05)')+'.sav'
    ...
    vec_norm_sh = get_norm_shock3(pos_mse,ind=ind_norm)
         ...
         for ia=0,15 do begin
            ...
            ;; convert 8 points into the bve coordinate system 
             if not keyword_set(mso) then begin
              ...
              if keyword_set(rmv_ring) then if  vdist_from_ring le vproton_perp_tot*1.5 then counts2[ie,ip,ia,*]=0.  
             endif           
