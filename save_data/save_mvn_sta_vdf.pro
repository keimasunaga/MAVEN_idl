...
function get_local_field,tspan,orbit=orbit
...
pro save_mvn_sta_vdf,orbit,trange=trange,save=save,event=event,_extra=extra
...
;    plot_sta_vdf,trange=tspan,/local,/show,units='df',sta_apid='d0',mult='4,3',panel_mode=1,/remove,vdf_sav=vdf_sav
    ...
    if keyword_set(event) then png_path = SAVE_LOC+'/maven/png/vdf/event/'+string(orbit,format='(i05)') $
                          else png_path = SAVE_LOC+'/maven/png/vdf/oxy/'+string(orbit,format='(i05)')                          
    if file_test(png_path) eq 0 then file_mkdir,png_path
    makepng,png_path+'/vdf_'+date_s,window=1

    if keyword_set(save) then begin
     if keyword_set(event) then save_path = SAVE_LOC_HEAVY+'/maven/sav/vdf/event/'+string(orbit,format='(i05)') $
                           else save_path = SAVE_LOC_HEAVY+'/maven/sav/vdf/oxy/'+string(orbit,format='(i05)')
