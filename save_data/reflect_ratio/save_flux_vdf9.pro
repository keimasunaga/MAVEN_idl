...
function get_local_field,tspan,orbit=orbit,highres=highres
  ...
  if keyword_set(highres) then begin
    ex_b = tplot_exist('mvn_B_1sec_MAVEN_MSO_smoothed')
    ex_v = tplot_exist('mvn_'+swiname+'_velocity_mso_smoothed')
    if ex_b eq 0 then tsmooth_in_time, 'mvn_B_1sec_MAVEN_MSO',360
    if ex_v eq 0 then tsmooth_in_time, 'mvn_'+swiname+'_velocity_mso',360
    get_data,'mvn_B_1sec_MAVEN_MSO_smoothed',data=d_b
    get_data,'mvn_'+swiname+'_velocity_mso_smoothed',data=d_v
    idx_b = nn('mvn_B_1sec_MAVEN_MSO_smoothed',mean(tspan,/double))
    idx_v = nn('mvn_'+swiname+'_velocity_mso_smoothed',mean(tspan,/double))
    Vvec = total(d_v.y[idx_v,*],1);/(idx_v[1]-idx_v[0]+1.)
    Bvec = total(d_b.y[idx_b,*],1);/(idx_b[1]-idx_b[0]+1.)
...
pro save_flux_vdf9,orbit,trange=trange,png=png,ps2pdf=ps2pdf,ps2png=ps2png,save=save,event=event,highres=highres,_extra=extra,get_apid=get_apid,$
