pro save_mvn_sw_info_bk,append_tvar=append_tvar
   ...
   if not keyword_set(append_tvar) then begin
     ...
     get_data,'mvn_swim_velocity_mso',data=vel
     get_data,'mvn_swim_density',data=dens
     get_data,'mvn_swim_temperature_mso',data=temp
     get_data,'mvn_swim_pressure',data=pres
     get_data,'mvn_swim_pdy',data=pdy         
 ...
 ;    get_data,'mvn_B_1sec',data=mag_sc
     ...
     idx_vel_in = nn('mvn_swim_velocity_mso',trange_in) & idx_vel_out = nn('mvn_swim_velocity_mso',trange_out)
     idx_dens_in = nn('mvn_swim_density',trange_in) & idx_dens_out = nn('mvn_swim_density',trange_out)
     idx_temp_in = nn('mvn_swim_temperature_mso',trange_in) & idx_temp_out = nn('mvn_swim_temperature_mso',trange_out)
     idx_pres_in = nn('mvn_swim_pressure',trange_in) & idx_press_out = nn('mvn_swim_pressure',trange_out)
     idx_pdy_in = nn('mvn_swim_pdy',trange_in) & idx_pdy_out = nn('mvn_swim_pdy',trange_out)
    ...
    ; mag_sc_in = avg(mag.y[idx_mag_sc_in[0]:idx_mag_in[1],*],0) & mag_sc_out = avg(mag.y[idx_mag_sc_out[0]:idx_mag_sc_out[1],*],0)
     ...
     if not keyword_set(time_avg) then time_avg  =15d
     tspan_avg = time_avg*60d ;15 minutes
     get_data,'mvn_swim_velocity_mso_1',data=vel1 & get_data,'mvn_swim_velocity_mso_2',data=vel2
     get_data,'mvn_swim_density_1',data=dens1 & get_data,'mvn_swim_density_1',data=dens2
     get_data,'mvn_swim_temperature_mso_1',data=temp1 & get_data,'mvn_swim_temperature_mso_2',data=temp2
     get_data,'mvn_swim_pressure_1',data=pres1 & get_data,'mvn_swim_pressure_2',data=pres2
     get_data,'mvn_swim_pdy_1',data=pdy1 & get_data,'mvn_swim_pdy_2',data=pdy2 
 ...
 ;    get_data,'mvn_B_1sec_1',data=mag_sc1 &  get_data,'mvn_B_1sec_2',data=mag_sc
     ...
     store_data,'mvn_swim_velocity_mso',data={x:[vel1.x,vel2.x],y:[vel1.y,vel2.y]}
     store_data,'mvn_swim_density',data={x:[dens1.x,dens2.x],y:[dens1.y,dens2.y]}
     store_data,'mvn_swim_temperature_mso',data={x:[temp1.x,temp2.x],y:[temp1.y,temp2.y]}
     store_data,'mvn_swim_pressure',data={x:[pres1.x,pres2.x],y:[pres1.y,pres2.y]}
     store_data,'mvn_swim_pdy',data={x:[pdy1.x,pdy2.x],y:[pdy1.y,pdy2.y]}
