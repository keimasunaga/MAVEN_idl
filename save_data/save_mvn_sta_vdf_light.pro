...
pro save_mvn_sta_vdf_light,orbit,save=save,_extra=extra
  ...
  for it = 0,n_elements(time)-1 do begin
    ...
    sta_getdat_bgrm_light,dat_c6,dat_c6_new,ratio=1
    ...
    sta_getdat_bgrm_light,dat,dat_new,ratio=1
...
;    plot_sta_vdf,trange=tspan,/local,/show,units='df',sta_apid='d0',mult='4,3',panel_mode=1,/remove,vdf_sav=vdf_sav
    plot_sta_vdf_light,dat=dat_new,/local,/show,units='eflux',sta_apid='d0',mult='4,4',panel_mode=1,nbin=250,vres=10,$
                 /remove,vdf_sav=vdf_sav,_extra=extra,mass_assmpt=4.,mass_range=[3,5];[12,20]
    ...
    png_path = SAVE_LOC+'/maven/png/vdf/herium/'+string(orbit,format='(i05)')
    if file_test(png_path) eq 0 then file_mkdir,png_path
    makepng,png_path+'/vdf_'+date_s,window=1

    if keyword_set(save) then begin
     save_path = SAVE_LOC+'/maven/sav/vdf/herium/'+string(orbit,format='(i05)')
