;+
; :Description:
;    This routine loads and tplots time series of PFP package (SEP, SWEA, STATIC, SWIA, MAG) and MAVEN pos (ALT and SZA) 
;    Also, this routine can save png image and tplot variables by setting keywords.
;    Used to produce orbital qlplots.
;    
; ${parameters}
;
; ${keywords}
...
; sorb: start orbit number
; norb: number of orbits
; png: save png image
; save: save tplot variables
; notplot: do not tplot
; 
; ${Return values}
;
; ${Related routines}
; tplot_mvn_pfp_daily -> daily qlplot 
;
...
pro tplot_mvn_pfp,sd=sd,ndays=ndays,sorb=sorb,norb=norb,png=png,save=save,notplot=notplot
...
;       t_rename,'MAVEN_POS_(Mars-MSO)_z','Zmso'
     ...
     ;; static load
       ...
       sta_c6_bgrm, eflux, counts, /overwrite ;;comment out when not need to tplot c6 data
       mvn_sta_l2_tplot
       sta_c6_bgrm_kei, eflux, counts, energy, time_c6 ,mass_range=[12,20], /overwrite ;;comment out when not need to tplot c6 data
       mvn_sta_l2_tplot_c6_kei ;;comment out when not need to tplot c6 data
     ...
     ;; swia load
       ...
       if sdate eq edate then begin  ;;start and end in the same day
          ...
          if idx_ca eq 1 then begin
            ...
            if idx_time_ca[0] ne idx_time_ca[1] then begin              
              ...
              ;;if moment data has only one element, we need to make it two elements becaues we can't tplot only one element             
              get_data,'mvn_swica_en_eflux',data=eflux_ca
              y = findgen(2,n_elements(eflux_ca.v[0,*])) & y[*] = !values.F_NAN 
              store_data,'mvn_swica_en_eflux',data={x:[st,et],y:y, v:eflux_ca.v[0:1,*], ylog:eflux_ca.ylog, zlog:eflux_ca.zlog, spec:eflux_ca.spec, no_interp:eflux_ca.no_interp, yrange:eflux_ca.yrange, ystyle:eflux_ca.ystyle, zrange:eflux_ca.zrange, ytitle:eflux_ca.ytitle, ztitle:eflux_ca.ztitle}             
              get_data,'mvn_swica_velocity_mso',data=vel_ca 
              y = findgen(2,n_elements(vel_ca.y[0,*])) & y[*] = !values.F_NAN  
              store_data,'mvn_swica_velocity_mso',data={x:[st,et],y:y}              
              get_data,'mvn_swica_density',data=dens_ca 
              y = findgen(2) & y[*] = !values.F_NAN 
              store_data,'mvn_swica_density',data={x:[st,et],y:y}             
              get_data,'mvn_swica_temperature',data=temp_ca 
              y = findgen(2,n_elements(temp_ca.y[0,*])) & y[*] = !values.F_NAN 
              store_data,'mvn_swica_temperature',data={x:[st,et],y:y}             
              get_data,'mvn_swica_pressure',data=press_ca
              y = findgen(2,n_elements(press_ca.y[0,*])) & y[*] = !values.F_NAN 
              store_data,'mvn_swica_pressure',data={x:[st,et],y:y}              
              get_data,'mvn_swica_pdy',data=pdy_ca
              y = findgen(2) & y[*] = !values.F_NAN 
              store_data,'mvn_swica_pdy',data={x:[st,et],y:y}             
              idx_ca = 0
            endelse          
          endif
          ...
          if idx_cs eq 1 then begin
            ...
            if idx_time_cs[0] ne idx_time_cs[1] then begin  
              ...
              ;;if moment data has only one element, we need to make it two elements becaues we can't tplot only one element
              get_data,'mvn_swics_en_eflux',data=eflux_cs
              y = findgen(2,n_elements(eflux_cs.v[0,*])) & y[*] = !values.F_NAN
              store_data,'mvn_swics_en_eflux',data={x:[st,et],y:y, v:eflux_cs.v[0:1,*], ylog:eflux_cs.ylog, zlog:eflux_cs.zlog, spec:eflux_cs.spec, no_interp:eflux_cs.no_interp, yrange:eflux_cs.yrange, ystyle:eflux_cs.ystyle, zrange:eflux_cs.zrange, ytitle:eflux_cs.ytitle, ztitle:eflux_cs.ztitle}
              get_data,'mvn_swics_velocity_mso',data=vel_cs
              y = findgen(2,n_elements(vel_cs.y[0,*])) & y[*] = !values.F_NAN
              store_data,'mvn_swics_velocity_mso',data={x:[st,et],y:y}
              get_data,'mvn_swics_density',data=dens_cs
              y = findgen(2) & y[*] = !values.F_NAN
              store_data,'mvn_swics_density',data={x:[st,et],y:y}
              get_data,'mvn_swics_temperature',data=temp_cs
              y = findgen(2,n_elements(temp_cs.y[0,*])) & y[*] = !values.F_NAN
              store_data,'mvn_swics_temperature',data={x:[st,et],y:y}
              get_data,'mvn_swics_pressure',data=press_cs
              y = findgen(2,n_elements(press_cs.y[0,*])) & y[*] = !values.F_NAN
              store_data,'mvn_swics_pressure',data={x:[st,et],y:y}
              get_data,'mvn_swics_pdy',data=pdy_cs
              y = findgen(2) & y[*] = !values.F_NAN
              store_data,'mvn_swics_pdy',data={x:[st,et],y:y}
              idx_cs = 0
            endelse
     ...
     ;; mag load
       ...
       btag = tplot_exist('mvn_B_1sec_MAVEN_MSO')
       if btag eq 0 then goto,nomag
