...
function get_local_field_dir,tspan
  env = init_env()
  SAVE_LOC = env.SAVE_LOC
  mtime = mean(tspan)
  orbit = round(mvn_orbit_num(time=mtime))
  ...
  if time_idx_b[1] - time_idx_b[0] lt (tspan[1]-tspan[0])/2d or time_idx_v[1] - time_idx_v[0] lt (tspan[1]-tspan[0])/2d then return,{B:[0,0,0],V:[0,0,0],E:[0,0,0]}

  Vvec = mean(d_v.y[idx_v[0]:idx_v[1],*],dimension=1) & Vvec_std = stddev(d_v.y[idx_v[0]:idx_v[1],*],dimension=1)
  Bvec = mean(d_b.y[idx_b[0]:idx_b[1],*],dimension=1) & Bvec_std = stddev(d_b.y[idx_b[0]:idx_b[1],*],dimension=1)
